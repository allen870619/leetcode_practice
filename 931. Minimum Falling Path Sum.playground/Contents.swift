class Solution {
    func minFallingPathSum(_ matrix: [[Int]]) -> Int {
        ...
        var save = Array(repeating: Array(repeating: -1, count: col), count: matrix.count)

        func calNext(_ r: Int, _ c: Int, total: Int) -> Int {
            if r == matrix.count {
                return total
            }
            if c < 0 || c >= col {
            ...
            if save[r][c] != -1 {
                return total + save[r][c]
            }

            save[r][c] = min(calNext(r + 1, c - 1, total: matrix[r][c]),
                             calNext(r + 1, c, total: matrix[r][c]),
                             calNext(r + 1, c + 1, total: matrix[r][c]))
            return total + save[r][c]
        }

        var result = calNext(0, 0, total: 0)
        for i in 1 ..< col {
            result = min(result, calNext(0, i, total: 0))
        }
        return result
    }
}
